...
DOCKER_BRANCH_IMAGE_NAME="${DOCKER_PROJECT}:with-data-${DOCKER_BRANCH_IMAGE_VERSION}"
...
DOCKER_TAG_IMAGE_NAME="${DOCKER_PROJECT}:with-data-${DOCKER_TAG_IMAGE_VERSION}"

# build image and login to docker hub
docker build -t $DOCKER_PROJECT . -f Dockerfile.with-data
